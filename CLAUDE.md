...
1. `.kiro/specs/<spec名>/task.md` — 何から実装するか（タスクリスト）
2. `.kiro/specs/<spec名>/design.md` — どう作るか（インターフェース・ロジック・設計の理由）
3. `.kiro/specs/<spec名>/requirements.md` — 何を作るか（ユーザーストーリー・受け入れ基準）
...
3. 実装完了後、`design.md` のテスト方針に従いテストを書く
4. 全タスク完了後、PR を作成する
...
- main ブランチに直接コミットしない
- テストを省略しない。design.md にテスト方針が記載されている場合は必ず実装する
